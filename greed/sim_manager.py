...
class SimulationManager:
    ...
    @property
    def pruned(self) -> List[SymbolicEVMState]:
        """
        Returns:
            All the pruned states (states that met the `prune` condition)
        """
        return self.stashes['pruned']
    ...
    def move(self, from_stash: str, to_stash: str, filter_func: Callable[[SymbolicEVMState], bool] = lambda s: True):
        ...
        for s in list(self.stashes[from_stash]):
            if filter_func(s):
                ...
                if to_stash == 'pruned':
                    SimulationManager.pruned_count += 1
    ...
    def step(self, find: Callable[[SymbolicEVMState], bool] = lambda s: False,
        ...
        for state in self.active:
            try:
                # Check if the state has crossed the from_stmt

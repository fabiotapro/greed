...
def parse_tac_ir(tac_code, function_set, oracle_functions):
    ...
    return function_callers, oracle_callers

def get_relevant_functions(function_callers, oracle_callers):
    """
    Recursively finds all function callers for the oracle callers.
    """
    ...
    for function in oracle_callers:
        recurse(function)
...
def load_files(contract_tac_path, function_csv_path, oracles_csv_path):
    ...
    return function_set, tac_code, oracle_functions
...
def main(folder_path):
    ...
    oracles_csv_path = "./test-space/oracle-functions.csv"

    function_set, tac_code, oracle_functions = load_files(contract_tac_path, function_csv_path, oracles_csv_path)

    function_callers, oracle_callers = parse_tac_ir(tac_code, function_set, oracle_functions)
    print(f"Function callers: {function_callers}")
    print(f"Oracle callers: {oracle_callers}")

    show_call_graph(function_callers)

    # Recursively find all function callers for the oracle callers
    # This obtains all the 'relevant' functions to symbolically execute
    relevant_functions = get_relevant_functions(function_callers, oracle_callers)
